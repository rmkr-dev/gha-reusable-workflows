...
Copy these into a caller repository under `.github/workflows/`. Prefer a version tag (`@v0.5.2`) once published; use `@main` only while iterating.
...
jobs:
  analyze:
    uses: rmkr-dev/gha-reusable-workflows/.github/workflows/codeql.yml@v0.5.2
    with:
      languages: '["python"]'
...
jobs:
  review:
    uses: rmkr-dev/gha-reusable-workflows/.github/workflows/dependency-review.yml@v0.5.2
    with:
...
jobs:
  sbom:
    uses: rmkr-dev/gha-reusable-workflows/.github/workflows/sbom.yml@v0.5.2
    with:
      path: .
      artifact-name: sbom
...
jobs:
  image:
    uses: rmkr-dev/gha-reusable-workflows/.github/workflows/docker-build.yml@v0.5.2
    with:
      context: .
      file: Dockerfile
      image-name: myapp
...
jobs:
  release:
    uses: rmkr-dev/gha-reusable-workflows/.github/workflows/release-tag.yml@v0.5.2
    with:
      tag: ${{ inputs.tag }}
      generate-notes: true
...
| `@v0.5.2` | Reproducible pin to the latest minor release |
